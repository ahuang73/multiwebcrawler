...
struct hsearch_data *hash_data; //shared hash table
Queue *q;
Queue *q_png_urls;



int png_total = 0; //keeps track of the # of png files downloaded(TODO: PNG Validation)
int t = 1; 
int n = 1;
int log_urls = 0;
...
void insert_into_hash_table(char *url) //NEW
{
    ENTRY e;
    ENTRY *entry_p;
    e.key = malloc(256 * sizeof(char)); //create a copy or the hashtable won't work correctly
    strcpy(e.key, url);

    pthread_mutex_lock(&mutex);
    int contains = hsearch_r(e, FIND, &entry_p, hash_data);
    pthread_mutex_unlock(&mutex);
    if (!contains) //checking if there's not already an entry
    {
        pthread_mutex_lock(&mutex);
        enqueue(q, e.key); 
        if (log_urls){
        ...
        hsearch_r(e, ENTER, &entry_p, hash_data);
...
int find_http(char *buf, int size, int follow_relative_links, const char *base_url)
{
    ...
    if (result)
    {
        ...
        for (i = 0; i < nodeset->nodeNr; i++)
        {
            ...
            {
                //printf("href: %s\n", href); // Andy - this is where the links are printed
                char temp_url[256];
                sprintf(temp_url, "%s", href);
                insert_into_hash_table(temp_url);
            }
            xmlFree(href);
        }
        xmlXPathFreeObject(result);
...
int process_png(CURL *curl_handle, RECV_BUF *p_recv_buf)
{
    ...
    {
        pthread_mutex_lock(&mutex);
        sprintf(fname, "./output_%d_%d.png", p_recv_buf->seq, png_seq_ids[p_recv_buf->seq]);
        write_success = write_file(fname, p_recv_buf->buf, p_recv_buf->size);
        isValidPng = validate_png(fname);
        if (isValidPng) //check if is a valid png AND if it's unique.
        {
            
            png_seq_ids[p_recv_buf->seq] = p_recv_buf->seq;
        ...
        if (!isValidPng) //remove invalid pngs but don't remove rewrites of identical pngs
        {
            remove(fname);
        }
        pthread_mutex_unlock(&mutex);
...
int process_data(CURL *curl_handle, RECV_BUF *p_recv_buf)
{
    ...
    {
        //printf("Content-Type: %s, len=%ld\n", ct, strlen(ct)); //Andy - length is misleading, just the length of the content type string
    }
    else
    {
...
int curl_xml_main(int argc, char **argv)
{
    CURL *curl_handle;
    CURLcode res;
    char url[256];
